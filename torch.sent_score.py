...
if __name__ == "__main__":
    ...
    train_iter = DataLoader(_train, batch_size=32, shuffle=True)
    test_iter = DataLoader(_test, batch_size=1, shuffle=False)

    batch_size = 64
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = LinearNet(d_input=4, d_out=1, dropout_p=0.3)
    ...
    def sent_train(e):
        ...
        src = src.to(device)
        trg = trg.to(device).view(trg.size(0), 1).type(torch.float32)
        y_predict = e.model(src)
        #trg = F.one_hot(trg).to(device)
    ...
    def sent_eval(e):
        ...
        #import pdb; pdb.set_trace()
        p = 1 if y_predict.squeeze().item() > tt else 0
        return torch.tensor([[p]]).to(device), trg.view(1, -1)
